...
export const studentService = {
    ...
    createInstance: async (name, description, ownerId) => {
    ...
    insertDocument: async (instanceId, collectionName, document, databaseName) => {
        try {
            ...
            console.log("Insert document request:", JSON.stringify(requestBody, null, 2));
    ...
    executeQuery: async (instanceId, query, collectionName, databaseName) => {
        try {
            ...
            const requestBody = {
                ...
                collectionName: collectionName || studentService.extractCollectionName(query),
...
export const mongoQueryConverter = {
    ...
    extractFindFilter: (query) => {
        // Buscar el contenido dentro de find()
        const findMatch = query.match(/\.find\(\s*({[^}]*}(?:\s*,\s*{[^}]*})?)\s*\)/);
        if (!findMatch) {
            return "{}"; // Filtro vacío
        }
    ...
    extractAggregatePipeline: (query) => {
